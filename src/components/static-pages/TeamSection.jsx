import React from 'react';

const TeamSection = () => {
    const teamMembers = [
        {
            id: 1,
            name: "Eade Marren",
            position: "Chief Executive",
            image: "/images/team/team-1.webp"
        },
        {
            id: 2,
            name: "Savannah Ngueen",
            position: "Operational Talent",
            image: "/images/team/team-2.webp"
        },
        {
            id: 3,
            name: "Kristin Watson",
            position: "Municipality Legal",
            image: "/images/team/team-3.webp"
        },
        {
            id: 4,
            name: "Darlene Robertson",
            position: "Business Director",
            image: "/images/team/team-4.webp"
        }
    ];

    return (
        <section className="bg-[#d8e5e5] py-20 relative overflow-hidden">
            {/* Pattern Background */}
            <div
                className="absolute inset-0 bg-repeat opacity-[1.5]"
                style={{
                    backgroundImage: 'url(/images/team/patern.png)',
                    backgroundRepeat: 'repeat',
                    backgroundSize: '200px 200px',
                    backgroundPosition: 'center'
                }}
            ></div>

            {/* Background Rack Design - Top Left */}
            <div className="absolute top-0 left-0 w-64 h-64 opacity-5">
                <div className="absolute top-10 left-10 w-48 h-48 border-2 border-gray-400 rounded-lg transform rotate-12"></div>
                <div className="absolute top-16 left-16 w-36 h-36 border-2 border-gray-400 rounded-lg transform rotate-6"></div>
                <div className="absolute top-24 left-24 w-24 h-24 border-2 border-gray-400 rounded-lg"></div>
            </div>

            {/* Background Rack Design - Bottom Right */}
            <div className="absolute bottom-0 right-0 w-64 h-64 opacity-5">
                <div className="absolute bottom-10 right-10 w-48 h-48 border-2 border-gray-400 rounded-lg transform -rotate-12"></div>
                <div className="absolute bottom-16 right-16 w-36 h-36 border-2 border-gray-400 rounded-lg transform -rotate-6"></div>
                <div className="absolute bottom-24 right-24 w-24 h-24 border-2 border-gray-400 rounded-lg"></div>
            </div>

            <div className="container mx-auto px-4 relative z-10">
                {/* Header Section */}
                <div className="text-center mb-12">
                    {/* Meet Our Team with icon */}
                    <div className="flex items-center justify-center mb-4">
                        <svg className="w-5 h-5 text-gray-600 mr-2" fill="currentColor" viewBox="0 0 20 20">
                            <path d="M13 6a3 3 0 11-6 0 3 3 0 016 0zM18 8a2 2 0 11-4 0 2 2 0 014 0zM14 15a4 4 0 00-8 0v3h8v-3zM6 8a2 2 0 11-4 0 2 2 0 014 0zM16 18v-3a5.972 5.972 0 00-.75-2.906A3.005 3.005 0 0119 15v3h-3zM4.75 12.094A5.973 5.973 0 004 15v3H1v-3a3 3 0 013.75-2.906z"/>
                        </svg>
                        <span className="text-sm font-medium text-gray-600">Meet Our Team</span>
                    </div>

                    {/* Main Heading */}
                    <h2 className="text-4xl font-bold text-gray-900 mb-6 leading-tight">
                        Success <span className="text-[#2a9d8f]">Stories</span> Fuel<br />
                        <span className="text-gray-700">our Innovation.</span>
                    </h2>
                    <div className="w-24 h-0.5 bg-gray-400 mx-auto mt-8"></div>
                </div>

                {/* Team Members Grid */}
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 max-w-5xl mx-auto">
                    {teamMembers.map((member) => (
                        <div
                            key={member.id}
                            className="group relative"
                        >
                            {/* Team Member Image Container */}
                            <div className="relative overflow-hidden rounded-lg mb-4 bg-black bg-opacity-15 h-80">
                                {/* Team Member Image */}
                                <img
                                    src={member.image}
                                    alt={member.name}
                                    className="w-full h-80 object-cover transition-transform duration-300 group-hover:scale-110"
                                />

                                {/* Hover Overlay - Black Blur Effect */}
                                <div className="absolute inset-0 bg-black bg-opacity-60 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-center justify-center">
                                    {/* Social Buttons */}
                                    <div className="flex space-x-4 transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
                                        <button className="w-10 h-10 bg-white bg-opacity-20 rounded-full flex items-center justify-center hover:bg-opacity-30 transition-all duration-200">
                                            <svg className="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 24 24">
                                                <path d="M24 4.557c-.883.392-1.832.656-2.828.775 1.017-.609 1.798-1.574 2.165-2.724-.951.564-2.005.974-3.127 1.195-.897-.957-2.178-1.555-3.594-1.555-3.179 0-5.515 2.966-4.797 6.045-4.091-.205-7.719-2.165-10.148-5.144-1.29 2.213-.669 5.108 1.523 6.574-.806-.026-1.566-.247-2.229-.616-.054 2.281 1.581 4.415 3.949 4.89-.693.188-1.452.232-2.224.084.626 1.956 2.444 3.379 4.6 3.419-2.07 1.623-4.678 2.348-7.29 2.04 2.179 1.397 4.768 2.212 7.548 2.212 9.142 0 14.307-7.721 13.995-14.646.962-.695 1.797-1.562 2.457-2.549z"/>
                                            </svg>
                                        </button>
                                        <button className="w-10 h-10 bg-white bg-opacity-20 rounded-full flex items-center justify-center hover:bg-opacity-30 transition-all duration-200">
                                            <svg className="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 24 24">
                                                <path d="M22.46 6c-.77.35-1.6.58-2.46.69.88-.53 1.56-1.37 1.88-2.38-.83.5-1.75.85-2.72 1.05C18.37 4.5 17.26 4 16 4c-2.35 0-4.27 1.92-4.27 4.29 0 .34.04.67.11.98C8.28 9.09 5.11 7.38 3 4.79c-.37.63-.58 1.37-.58 2.15 0 1.49.75 2.81 1.91 3.56-.71 0-1.37-.2-1.95-.5v.03c0 2.08 1.48 3.82 3.44 4.21a4.22 4.22 0 0 1-1.93.07 4.28 4.28 0 0 0 4 2.98 8.521 8.521 0 0 1-5.33 1.84c-.34 0-.68-.02-1.02-.06C3.44 20.29 5.7 21 8.12 21 16 21 20.33 14.46 20.33 8.79c0-.19 0-.37-.01-.56.84-.6 1.56-1.36 2.14-2.23z"/>
                                            </svg>
                                        </button>
                                        <button className="w-10 h-10 bg-white bg-opacity-20 rounded-full flex items-center justify-center hover:bg-opacity-30 transition-all duration-200">
                                            <svg className="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 24 24">
                                                <path d="M12.017 0C5.396 0 .029 5.367.029 11.987c0 5.079 3.158 9.417 7.618 11.174-.105-.949-.199-2.403.042-3.441.219-.937 1.407-5.965 1.407-5.965s-.359-.719-.359-1.782c0-1.668.967-2.914 2.171-2.914 1.023 0 1.518.769 1.518 1.69 0 1.029-.655 2.567-.992 3.992-.285 1.193.6 2.165 1.775 2.165 2.128 0 3.768-2.245 3.768-5.487 0-2.861-2.063-4.869-5.008-4.869-3.41 0-5.409 2.562-5.409 5.199 0 1.033.394 2.143.889 2.741.099.12.112.225.085.345-.09.375-.293 1.199-.334 1.363-.053.225-.172.271-.402.165-1.495-.69-2.433-2.878-2.433-4.646 0-3.776 2.748-7.252 7.92-7.252 4.158 0 7.392 2.967 7.392 6.923 0 4.135-2.607 7.462-6.233 7.462-1.214 0-2.357-.629-2.75-1.378l-.748 2.853c-.271 1.043-1.002 2.35-1.492 3.146C9.57 23.812 10.763 24.009 12.017 24.009c6.624 0 11.99-5.367 11.99-11.988C24.007 5.367 18.641.001.012.017z"/>
                                            </svg>
                                        </button>
                                    </div>
                                </div>
                            </div>

                            {/* Name and Position - Left Aligned with smaller sizes and padding */}
                            <div className="text-left px-2">
                                <h3 className="text-xl font-bold text-gray-900 mb-1 group-hover:text-gray-700 transition-colors">
                                    {member.name}
                                </h3>
                                <p className="text-base text-gray-600 font-light">
                                    {member.position}
                                </p>
                            </div>
                        </div>
                    ))}
                </div>
            </div>
        </section>
    );
};

export default TeamSection;